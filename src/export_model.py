...
def export_pipeline(
    ...
    if multilingual:
        model = UIEM.from_pretrained(model_path)
    else:
        model = UIE.from_pretrained(model_path)
    tokenizer = BertTokenizer.from_pretrained(model_path)
    rich.print("Export Tokenizer Config...")
    export_tokenizer(model_path, model_path)

    rich.print("Export ONNX Model...")
    ...
    rich.print(f"All good, model saved at: {save_path}")
...
def export_onnx(output_path: Union[Path, str], tokenizer: PreTrainedTokenizerBase, model: PreTrainedModel, device: torch.device, input_names: List[str], output_names: List[str], onnx_file: str = "inference.onnx"):
    with torch.no_grad():
        ...
        save_path = output_path / onnx_file
...
def export_tokenizer(model_path: Path, output_path: Path):
    for tokenizer_fine in ['tokenizer_config.json', 'special_tokens_map.json', 'vocab.txt']:
        file_from = model_path / tokenizer_fine
        file_to = output_path / tokenizer_fine

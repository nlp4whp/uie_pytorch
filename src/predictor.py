...
from .models.utils import get_bool_ids_greater_than, get_span, get_id_and_prob, cut_chinese_sent, dbc2sbc
...
class SchemaTree:
    ...
    def __repr__(self):
        return "{}".format({'name': self.name, 'children': self.children})
...
class UIEPredictor(object):
    ...
    def __init__(
        ...
        model: str,
        task_path: str,
        schema: Union[List[str], Dict[str, List[str]]],
        schema_lang: str = "zh",
        engine: str = 'pytorch',
        device: str = 'cpu',
        ...
        self._multilingual = True if model.startswith('uie-m-') else multilingual
        self._model = model
        self._engine = engine
    ...
    def _prepare_predictor(self):
        ...
        if self._multilingual:
            from .models import ErnieMTokenizerFast
            self._tokenizer = ErnieMTokenizerFast.from_pretrained(self._task_path)
        else:
            from transformers import BertTokenizerFast
            self._tokenizer = BertTokenizerFast.from_pretrained(self._task_path)

        if self._engine == 'pytorch':
            from .models import PyTorchInferBackend
        ...
        if self._engine == 'onnx':
            from .models import ONNXInferBackend
            # if (
            #     os.path.exists(os.path.join(self._task_path, "pytorch_model.bin"))
            #     and not os.path.exists(os.path.join(self._task_path, "inference.onnx"))
            # ):
            #     ## NOTE convert onnx model
    ...
    def _multi_stage_predict(self, datas: List[str]):
        """
    ...
    def _single_stage_predict(self, inputs):
        ...
        for lft in range(0, len(short_input_texts), self._batch_size):
            input_ids = encoded_inputs["input_ids"][lft: lft + self._batch_size]
            token_type_ids = encoded_inputs["token_type_ids"][lft: lft + self._batch_size]
            attention_mask = encoded_inputs["attention_mask"][lft: lft + self._batch_size]
            offset_maps = encoded_inputs["offset_mapping"][lft: lft + self._batch_size]
            if self._multilingual:
                input_ids = np.array(

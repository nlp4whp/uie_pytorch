...
from .models.utils import (
    SpanEvaluator, IEMapDataset, IEDataset, get_relation_type_dict, unify_prompt_name)
...
def do_eval(
    ...
    for key in class_dict.keys():
        if debug:
            test_ds = IEMapDataset(class_dict[key], tokenizer=tokenizer, max_seq_len=max_seq_len)
        else:
        ...
        precision, recall, f1 = evaluate(
            model, metric, test_data_loader, device)
        logger.info("-----------------------------")
        logger.info("Class Name: %s" % key)
        logger.info("Evaluation Precision: %.5f | Recall: %.5f | F1: %.5f" %
                    (precision, recall, f1))

    if debug and len(relation_type_dict.keys()) != 0:
        for key in relation_type_dict.keys():
            test_ds = IEMapDataset(
                relation_type_dict[key], tokenizer=tokenizer, max_seq_len=max_seq_len)

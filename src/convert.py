...
from ._model_dict import MODEL_MAP
...
def download_and_transform(
    src_root='paddlenlp',
    tgt_root='pytorch',
    tar_files=["pytorch_model.bin", "config.json", "tokenizer_config.json", "vocab.txt"],
    model_names=('uie-base', 'uie-medical-base', 'uie-mdeium'),
):
    """ 下载paddle模型并转为pytorch格式 """
    os.makedirs(src_root, exist_ok=True)
    os.makedirs(tgt_root, exist_ok=True)
    # [MODEL_MAP](https://aistudio.baidu.com/aistudio/modelsdetail?modelId=22)
    for ckpt in model_names:
        if ckpt not in MODEL_MAP:
            raise NotImplementedError(f"Only support model_name in {MODEL_MAP.keys()}")
        if not os.path.isdir(src_dir := os.path.join(src_root, ckpt)):
            _check_and_download(ckpt, save_home=src_root)
        if not all(
            os.path.exists(os.path.join(tgt_root, ckpt, _fname)) for _fname in tar_files
        ):
            _extract_and_convert(src_dir, output_dir=os.path.join(tgt_root, ckpt))
        else:
            logger.info(f"Model:{ckpt} exists")


def _check_and_download(input_model, save_home):
    if os.path.exists(input_model):
        logger.info(f"Model {input_model} exists ...")
        return
    ...
    for key, val in resource_file_urls.items():
        ...
        if not os.path.exists(file_path):
            ...
            #         f.write(base64data)
            download_path = _get_path_from_url(val, os.path.join(save_home, input_model))
            if download_path != file_path:
                shutil.move(download_path, file_path)


def _build_params_map(model_prefix='encoder', attention_num=12):
    """
...
def _extract_and_convert(input_dir, output_dir, verbose=False):
    ...
    weight_map = _build_params_map(attention_num=config['num_hidden_layers'])
    weight_map.update(_build_params_map(
    ...
    paddle_params = pickle.load(open(os.path.join(input_dir, 'model_state.pdparams'), 'rb'))
    del paddle_params['StructuredToParameterName@@']
    for weight_name, weight_value in paddle_params.items():
        transposed = ''
...
def _get_path_from_url(url, root_dir, check_exist=True, decompress=True):

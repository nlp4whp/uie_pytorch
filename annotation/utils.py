...
def convert_cls_examples(raw_examples, prompt_prefix="情感倾向", options=["正向", "负向"]):
    ...
    """
    examples = []
    rich.print("Converting doccano data...")
...
def convert_ext_examples(
    ...
    rich.print("Adding negative samples for first stage prompt...")
    ...
    if len(predicate_set) != 0:
        rich.print("Adding negative samples for second stage prompt...")
        if is_train:
            ...
            with tqdm(total=len(texts)) as pbar:
                for i, text in enumerate(texts):
                    ...
                    pbar.update(1)
            all_relation_examples = positive_examples + [{**e, **{'neg': True}} for e in negative_examples]
        else:
